...
const get_user = async (username: string, otherusername: string): Promise<Array<{ username: string, name: string, posts_count: number, posts: Array<any> }>> => {
	...
	const liked_by_user = likes.filter(like =>
		like.username === otherusername
	);

	const posts: any[] = posts_res.map(post => {
		const related_comments = comments.filter(comment => comment.post_id === post.id)
