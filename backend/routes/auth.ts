...
const register = async (fastify: FastifyInstance, opts: FastifyPluginOptions, next: any) => {
	fastify.post<{ Body: BodyUserType }>("/", {
		...
		const { username, name, email, dob, password, account_type, profile_pic } = req.body;
		...
		try {
			await sql.begin(async sql => {
				const statement = sql`
					...
					values
						(LOWER(${username}), ${name}, ${dob}, ${email}, ${account_type_string}, ${profile_pic});
					`;
				await statement;
				await sql`
					...
					values
						(LOWER(${username}), ${hash})
					`;
				jwt.sign({ username, name }, JWT_SECRET, async (err: any, token: any) => {
...
const login = (fastify: FastifyInstance, opts: FastifyPluginOptions, next: any) => {
